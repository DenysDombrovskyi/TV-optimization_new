...
def run_optimization(df, total_budget, goal, mode, buying_audiences, deviation_df):
    """
    ...
    if mode == 'per_sh':
        with st.spinner('Проводимо оптимізацію за кожним СХ...'):
            ...
            for sales_house, group_df in df.groupby('СХ'):
                ...
                group_df['Нижня межа'] = group_df['Стандартний бюджет'] * (1 - group_df['Мінімальне відхилення'] / 100)
                group_df['Верхня межа'] = group_df['Стандартний бюджет'] * (1 + group_df['Максимальне відхилення'] / 100)
        ...
        with st.spinner('Проводимо загальну оптимізацію...'):
            ...
            # Використовуємо задані користувачем відхилення
            df['Нижня межа'] = df['Стандартний бюджет'] * (1 - df['Мінімальне відхилення'] / 100)
            df['Верхня межа'] = df['Стандартний бюджет'] * (1 + df['Максимальне відхилення'] / 100)
...
if uploaded_file:
    try:
        ...
        if st.button("🚀 Запустити оптимізацію"):
            all_results = run_optimization(all_data.copy(), total_budget, goal, mode, buying_audiences, edited_deviation_df)
